...
function [] = solvelinear5(A,B,f, t0, xi1, p, rs,xs,ys, a,b,c);
figure();
daspect([1 1 1]);
hold on;
...
for i = 1:N
    ...
    x_opt = x_mopt;
    number = number +1;
    [t, y, te, xe, ie] = ode45(@(t, y) finalsystem(t, y, C, D, [v1(end); v2(end)], g), [s0 sspan(end)], xi1, options);
    for j = 2:size(y, 1)
        x_mopt(1, j) = y(j, 1);
        x_mopt(2, j) = y(j, 2);
...
if(flag)
    ...
    figure()
    plot(tspan(1:size(pppsi,2)), pppsi(1,1:end), tspan(1:size(pppsi,2)), pppsi(2,1:end));
    xlabel('t');
    ylabel('\psi(t)');
    legend('\psi_1(t)','\psi_2(t)');
    figure()
    plot(tspan(1:size(v_opt,2)), v_opt(1,1:end), tspan(1:size(v_opt,2)), v_opt(2,1:end));
    xlabel('t');
    ylabel('u(t)');
    legend('u1(t)','u2(t)');
end
